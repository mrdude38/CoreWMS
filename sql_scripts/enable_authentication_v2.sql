...
-- Complete Migration Script v2 (Idempotent)
-- Execute in Supabase SQL Editor
-- ============================================

-- STEP 1: Rename existing users table to warehouse_operators (only if users exists and warehouse_operators doesn't)
DO $$
BEGIN
    -- Check if 'users' exists and 'warehouse_operators' doesn't
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'users')
       AND NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'warehouse_operators') THEN
        ALTER TABLE users RENAME TO warehouse_operators;
        RAISE NOTICE 'Table users renamed to warehouse_operators';
    ELSIF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'warehouse_operators') THEN
        RAISE NOTICE 'Table warehouse_operators already exists, skipping rename';
    ELSE
        RAISE NOTICE 'No users table found to rename';
    END IF;
END $$;
...
-- STEP 7: Update foreign key constraints for entries (only if warehouse_operators exists)
DO $$
BEGIN
    -- Only update if warehouse_operators table exists
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'warehouse_operators') THEN
        -- Drop existing constraint if it exists
        IF EXISTS (SELECT 1 FROM information_schema.table_constraints
                   WHERE constraint_name = 'entries_received_by_fkey'
                   AND table_name = 'entries') THEN
        ...
        RAISE NOTICE 'Foreign key constraint updated for entries.received_by';
    ELSE
        RAISE NOTICE 'warehouse_operators table not found, skipping FK update';
    END IF;
END $$;
...
-- Enable RLS on warehouse_operators only if it exists
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'warehouse_operators') THEN
        ALTER TABLE warehouse_operators ENABLE ROW LEVEL SECURITY;
        RAISE NOTICE 'RLS enabled on warehouse_operators';
    END IF;
END $$;
...
-- STEP 13: Create RLS policies for warehouse_operators (only if table exists)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'warehouse_operators') THEN
        ...
        RAISE NOTICE 'RLS policies created for warehouse_operators';
    END IF;
END $$;
...
-- Migration completed - show status
SELECT
  'Authentication system enabled successfully!' as status,
  (SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_profiles') as user_profiles_exists,
  (SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'roles_permissions') as roles_permissions_exists,
  (SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'warehouse_operators') as warehouse_operators_exists;
